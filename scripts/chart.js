...
d3.json("data/data.json", function(error, data) {
  ...
  color.domain(d3.keys(data[0]).filter(function(key) { return key !== "animals"; }));
  ...
  x.domain(data.map(function(d) { return d.animals; }));
  y.domain([0, d3.max(
	  data, function(d) {
		  console.log(d.total)
		  return d.total;
  ...
  var state = svg.selectAll(".state")
    ...
    .enter().append("g")
      .attr("class", "g")
      .attr("transform", function(d) { return "translate(" + x(d.animals) + ",0)"; });
